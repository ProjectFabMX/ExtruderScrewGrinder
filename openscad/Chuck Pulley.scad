...
motor_shaft_diameter = 10; // (in mm)
toothed_part_length = 9; // [3:30] (in mm)
...
difference() {
    difference() {
	cylinder (h = 30, r=80/2, center = true, $fn=100);
	cylinder (h = 30, r=12.5, center = true, $fn=100);
        }
         if (no_of_nuts > 0){
          for(j=[1:no_of_nuts]){
              rotate([0,0,j*nut_angle+60]){
              translate ([0,-28,0 ]){
            cube([15,38,15],center=true);
            }
        }
    }
}
...
difference() {
	cylinder (h = 2.5, r=(45+13.5)/2, center = true, $fn=100);
	cylinder (h = 2.5, r=(45-13.5)/2, center = true, $fn=100);
        }
    }
    }
      translate([0,0,(base_height)+15]){
    cylinder (h = 8, r=15, center = true, $fn=100);
      }
    //hole for motor shaft
    translate([0,0,-1]){
      cylinder(r=motor_shaft/2,h=base_height + idler_height + toothed_part_length + retainer_height + 2,$fn=motor_shaft*30);
    }
          ...
          for(j=[1:no_of_nuts]){
            rotate([0,0,j*nut_angle]){
              translate([0,0,nut_elevation])rotate([90,0,0]){
                union(){
                  //entrance
                  translate([0,-base_height/4-0.5-4,motor_shaft/2+nut_depth/2+nut_shaft_distance]) cube([nut_width,base_height/2+1+6,nut_depth],center=true);
                  //nut
                  if ( nut_shape > 0 ){
                    // hex nut
                  ...
                  //grub screw hole
                  rotate([0,0,22.5])cylinder(r=screw_diameter/2,h=base_diameter/2+10,$fn=16);
                  translate([0,0.25,motor_shaft/2+nut_depth/2+nut_shaft_distance+15])rotate([0,0,90])cylinder(r=5/2,h=base_diameter/2+1000,$fn=6);
                }
              }
            }
          }
...
module GT2_5mm()
 ...
 // translate([62.494,0,-2]){
//import("C:/Users/jacob/Desktop/Angle Grinder Lathe/25GT2_2.stl");
//  }